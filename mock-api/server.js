...
app.post('/payments', (req, res) => {
  ...
  if (!accounts[account_id]) return res.status(404).json({ error: 'Account not found' });
  if (accounts[account_id].balance < amount) return res.status(400).json({ error: 'Insufficient funds' });
...
app.post('/refunds', (req, res) => {
  ...
  if (amount > payment.amount) return res.status(400).json({ error: 'Refund exceeds original payment' });
...
app.post('/transfers', (req, res) => {
  ...
  if (!from_account || !to_account) return res.status(400).json({ error: 'Account not found' });
  if (!amount || amount <= 0) return res.status(400).json({ error: 'Invalid transfer amount' });
  if (!accounts[from_account] || !accounts[to_account]) return res.status(404).json({ error: 'Account not found' });
  if (accounts[from_account].balance < amount) return res.status(400).json({ error: 'Insufficient funds' });
